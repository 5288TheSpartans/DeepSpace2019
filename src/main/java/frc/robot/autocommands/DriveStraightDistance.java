...
public class DriveStraightDistance extends Command {
	...
	public DriveStraightDistance(double distance) {
		...
		PID = new SpartanPID(RobotMap.DriveStraightP, RobotMap.DriveStraightI, RobotMap.DriveStraightD, RobotMap.DriveStraightFF);
	...
	protected void execute() {
		...
		// Output using the distanceScalar*(Maxspeed + error), effectively scaling down
		// both the speed and the error.
		Robot.drivetrain.setLeftPower(-speed - error);
		Robot.drivetrain.setRightPower(-speed + error);
	}
	...
	protected boolean isFinished() {
		...
		/*
		 * if(inWantedDistance > 0) { rv = distancetravelled >= inWantedDistance - .25;
		 * } else{ rv = distancetravelled <= inWantedDistance + .25; }
		 */
		if (deltaTime >= 5000) {
